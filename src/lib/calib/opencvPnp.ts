...
/**
 * OpenCV solvePnPRansac + LM refine via the sidecar. Same 3D↔projector
 * correspondences as the in-browser DLT; better R,t when cv2 is installed.
 */
export async function solveProjectorPnpOpenCv(input: {
  ...
  t?: Vec3;
  dist?: number[];
}): Promise<ProjectorCalibration | null> {
  ...
  try {
    const res = await fetch("/api/pnp", {
      ...
      body: JSON.stringify({
        ...
        R: input.R ? [...input.R] : undefined,
        t: input.t ? [...input.t] : undefined,
      }),
    });
    if (!res.ok) return null;
    const body = (await res.json()) as PnpBody;
